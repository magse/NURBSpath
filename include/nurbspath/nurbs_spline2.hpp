 ...
 * a separate 3D spline by projecting only this curve's control points.
...
public:
     ...
     * @throws std::overflow_error When the required knot count is not representable.
    ...
    /**
     * @brief Construct a standard open or closed 2D spline between two points.
     *
     * Control points are distributed uniformly from `start` to `end` with
     * `lerp`, all rational weights are one, and standard open-clamped knots
    ...
    nurbs_spline2(
        ...
        : nurbs_spline2(
              make_standard_definition(
                  start, end, control_point_count, degree),
              degree,
              closed,
              tolerance,
              distance(start, end)) {}
    ...
    /**
     * @brief Copy an array-backed 2D NURBS into ordinary vector storage.
     ...
     */
    explicit nurbs_spline2(const nurbs_arr_spline2<REAL>& spline);
     ...
     * control-point coordinates in point-major X/Y order, followed by all
    ...
    void set_parameters(const REAL* parameter_values) {
        if (parameter_values == nullptr) {
            throw std::invalid_argument(
                "2D NURBS parameter source pointer must not be null");
        }

        std::vector<point2<REAL>> candidate_control_points(
            control_points_.size());
        std::size_t offset = 0;
        for (point2<REAL>& point : candidate_control_points) {
     ...
     * @param index Zero-based control-point index.
     * @param point New finite 2D control point.
     * @throws std::out_of_range When index is outside the control-point vector.
    ...
    void set_control_point(std::size_t index, const point2<REAL>& point) {
        std::vector<point2<REAL>> updated = control_points_;
     ...
     *
     * @param control_points New finite 2D control points.
     * @throws std::invalid_argument When the resulting definition or seam is invalid.
     * @throws std::domain_error When endpoint evaluation has near-zero homogeneous weight.
     */
    ...
    void set_standard_knots(REAL s_start, REAL s_end) {
        if (!std::isfinite(s_start) || !std::isfinite(s_end) ||
            ...
            throw std::invalid_argument(
                "standard 2D NURBS knot domain must have finite positive length");
        }
        ...
        for (std::size_t span = 1; span < span_count; ++span) {
            ...
            if (!(interior_knot < s_end) ||
                ...
                throw std::invalid_argument(
                    "standard 2D NURBS knot spans must be numerically usable");
            }
     ...
     */
    [[nodiscard]] const point2<REAL>& get_end() const noexcept { return end_; }
     ...
     * `abs(first.cross(second)) / length(first)^3`. It is independent of the
     * spline parameterization while the analytic derivatives remain
     * representable in `REAL`, and has units of inverse 2D world length. At
     * an internal knot without second-derivative continuity, the result uses
     * the right-hand nonempty span; `s_max()` uses the left-hand nonempty span.
     *
     * @param s Finite parameter in the active knot domain.
    ...
    /**
     * @brief Get the total length of the 2D control polygon.
     *
     * The result is the sum of Euclidean distances between consecutive
     * control points. No additional last-to-first segment is inserted. This
     * is an exact length for the stored control polygonal chain and a simple
     * estimate of the spline's arc length; weights, knots, and sampling do not
     * affect it.
     *
     * @return Control-polygon length in 2D world units.
     */
    [[nodiscard]] REAL get_polygon_length() const noexcept {
        REAL length = REAL(0);
        for (std::size_t index = 1; index < control_points_.size(); ++index) {
            length +=
                distance(control_points_[index - 1], control_points_[index]);
        }
        return length;
    }
    ...
    /**
     * @brief Build an open or closed unit-weight interpolating 2D spline.
     * @param samples Positions to interpolate exactly.
     * @param arc_length_parameters Strictly increasing station for every sample.
     * @param requested_degree Desired positive degree, reduced when necessary.
     * @param closed True to require coincident active-domain endpoints.
    ...
    [[nodiscard]] static nurbs_spline2 interpolate(
        ...
        if (closed && distance(samples.front(), samples.back()) > tolerance) {
            throw std::invalid_argument(
                "closed 2D interpolation requires coincident first and final samples");
        }
...
private:
    struct standard_spline_definition {
        std::vector<point2<REAL>> control_points;
        std::vector<REAL> weights;
        std::vector<REAL> knots;
    };

    [[nodiscard]] static standard_spline_definition make_standard_definition(
        const point2<REAL>& start,
        const point2<REAL>& end,
        std::size_t control_point_count,
        std::size_t degree) {
    ...
    nurbs_spline2(
        standard_spline_definition definition,
        std::size_t degree,
        bool closed,
        REAL tolerance,
        REAL s_end)
    ...
    void validate_and_refresh() {
        validate_definition();
        const spline_derivatives2<REAL> start_values = derivatives_at(s_min());
        const spline_derivatives2<REAL> end_values = derivatives_at(s_max());
