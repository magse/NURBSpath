 ...
 * B-spline basis derivatives and the rational quotient rule. A repeated
 * spline evaluates translated copies of its fundamental period so that every
 * copy starts where the preceding copy ends.
 *
 * @tparam REAL Floating-point scalar type.
 */
template <std::floating_point REAL>
...
public:
     ...
     * @param control_points_value World-space control points.
     * @param weights_value Positive rational weight for every control point.
     * @param knots_value Nondecreasing knot vector.
     * @param degree_value Positive polynomial degree below control-point count.
     ...
     */
    nurbs_spline3(
        std::vector<point3<REAL>> control_points_value,
    ...
    /**
     * @brief Construct a closed or repeated 3D NURBS curve.
     * @param control_points_value World-space control points.
     * @param weights_value Positive rational weight for every control point.
     * @param knots_value Nondecreasing knot vector.
     * @param degree_value Positive polynomial degree below control-point count.
    ...
    nurbs_spline3(
        ...
        const spline_derivatives3<REAL> start_values =
            derivatives_at_base(period_s_min());
        const spline_derivatives3<REAL> period_end_values =
        ...
        if (is_periodic() && degree_ >= 2 &&
            ...
            throw std::invalid_argument(
                "periodic NURBS first derivatives must match at the seam");
        }
        ...
        if (is_periodic() && degree_ >= 3 &&
            (start_values.second - period_end_values.second).length() >
                second_tolerance) {
            throw std::invalid_argument(
                "periodic NURBS second derivatives must match at the seam");
        }
        if (period_count_ != 0 && !std::isfinite(s_max())) {
            throw std::invalid_argument(
                "finite repeated NURBS domain is not representable");
        }
        if (period_count_ != 0) {
            ...
            if (!std::isfinite(end_.x()) || !std::isfinite(end_.y()) ||
                !std::isfinite(end_.z())) {
                throw std::invalid_argument(
                    "finite repeated NURBS endpoint is not representable");
            }
        }
    }
     ...
     */
    [[nodiscard]] const point3<REAL>& get_end() const {
        if (period_count_ == 0) {
            throw std::domain_error("an unlimited NURBS path has no end");
        }
        return end_;
    }
    ...
    [[nodiscard]] spline_derivatives3<REAL> derivatives_at(REAL s) const {
        ...
        spline_derivatives3<REAL> result =
...
private:
    ...
    [[nodiscard]] spline_derivatives3<REAL> derivatives_at_base(
        ...
        // the scalar weight derivative w^(k).
        std::array<vector3<REAL>, 3> numerator{};
        ...
        for (std::size_t order = 0; order <= requested_order; ++order) {
            for (std::size_t local = 0; local <= degree_; ++local) {
                ...
                const REAL coefficient = derivatives[order][local] * weights_[control_index];
        ...
        const vector3<REAL> position_vector = numerator[0] / weight_derivative[0];
        vector3<REAL> first{};
        vector3<REAL> second{};
        if (degree_ >= 1) {
        ...
        return {
            point3<REAL>{position_vector.x(), position_vector.y(), position_vector.z()},
            first,
            second
        };
    }
...
public:
    ...
    [[nodiscard]] REAL approximate_arc_length(std::size_t segment_count = 512) const {
        ...
        if (period_count_ == 0) {
            throw std::domain_error(
                "cannot approximate the total length of an unlimited NURBS path");
        }
        REAL length = REAL(0);
    ...
    /**
     * @brief Build a closed or repeated unit-weight interpolating 3D spline.
     *
     ...
     *
     * @param samples World-space positions in one period, including both ends.
     * @param arc_length_parameters Strictly increasing station for each sample.
     * @param requested_degree Desired positive degree, reduced when necessary.
    ...
    [[nodiscard]] static nurbs_spline3 interpolate(
        ...
        if (closed && distance(samples.front(), samples.back()) > tolerance) {
            throw std::invalid_argument(
                "closed interpolation must repeat its first sample at the end");
        }
        ...
        if (period_count != 1) {
            ...
            if (unique_count < 2) {
                throw std::invalid_argument(
                    "periodic interpolation requires at least two unique samples");
            }
            ...
            const vector3<REAL> displacement = samples.back() - samples.front();
            ...
            std::vector<point3<REAL>> seed_controls(control_count);
            ...
            std::vector<REAL> z_values(unique_count);
            ...
            for (std::size_t row = 0; row < unique_count; ++row) {
                ...
                for (std::size_t local = 0; local <= degree; ++local) {
                    ...
                    if (expanded_index >= unique_count) {
                        ...
                        z_values[row] -= values[local] * displacement.z();
                    }
                }
            }
            ...
            std::vector<point3<REAL>> control_points(control_count);
            for (std::size_t index = 0; index < unique_count; ++index) {
        ...
        return nurbs_spline3(
            ...
            std::move(knots), degree, closed, std::size_t(1), tolerance);
    }
...
private:
    ...
    void validate_repeated_definition() const {
        ...
        if (unique_count <= degree_) {
            throw std::invalid_argument(
                "periodic NURBS requires more unique controls than its degree");
        }
        ...
        if (!(knots_[degree_ - 1] < knots_[degree_] &&
            ...
            throw std::invalid_argument(
                "periodic NURBS requires simple start and end knots");
        }
        ...
        for (std::size_t index = 0; index < degree_; ++index) {
            ...
            if ((current_displacement - control_displacement).length() >
                ...
                std::abs(weights_[index] - weights_[unique_count + index]) >
                    weight_tolerance) {
                throw std::invalid_argument(
                    "periodic NURBS must translate its first degree controls consistently and repeat their weights");
            }
        }
        ...
        for (std::size_t index = 0; index <= 2 * degree_; ++index) {
            ...
            if (std::abs(shifted - period) > parameter_tolerance) {
                throw std::invalid_argument(
                    "periodic NURBS knots must repeat after one period");
            }
        }
    }
    ...
    [[nodiscard]] parameter_mapping checked_parameter(REAL s) const {
        ...
        if (s < period_s_min() - tolerance_) {
            throw std::out_of_range(
                "NURBS parameter s is before the active path domain");
        }
    ...
    /// Algorithm A2.3 from The NURBS Book, specialized to at most order two.
    ...
    vector3<REAL> period_displacement_{};
    point3<REAL> start_{};
    point3<REAL> end_{};
};
